...
#include "feoct_include.hh"
#include "feoct_monit.hh"

void 
tree_monit::visit_octave_user_function (octave_user_function& item)
{
...
void 
tree_monit::visit_parameter_list (tree_parameter_list& item)
{
  ...
  indent += 2;
  int i = 0;
  tree_parameter_list::iterator p = item.begin();
  while (p != item.end ()){
...
void 
tree_monit::visit_statement_list (tree_statement_list& item)
{
  ...
  indent += 2;
  int i = 0;
  tree_statement_list::iterator p = item.begin();
  while (p != item.end ()){
...
void 
tree_monit::visit_statement (tree_statement& item) 
{
...
void
tree_monit::print_comment_list (octave_comment_list& item)
{
  fprintf (f, "%*s*(octave_comment_list *)%8.8p\n", 
	   indent, "", &item);

  int i = 0;
  octave_comment_list::iterator p = item.begin();
  while (p != item.end ()){
    indent += 2;
...
void 
tree_monit::print_comment (octave_comment_elt& item)
{
  fprintf (f, "%*s# %s\n", 
	   indent, "",
	   item.text ().c_str ());  
...
void
tree_monit::print_symbol_table (symbol_table& item)
{
...
// void
// tree_monit::print_symbol (symbol_record& item)
// {
...
void 
tree_monit::visit_break_command (tree_break_command& item) 
{
...
void 
tree_monit::visit_continue_command (tree_continue_command& item) 
{
...
void 
tree_monit::visit_return_command (tree_return_command& item) 
{
...
void 
tree_monit::visit_no_op_command (tree_no_op_command& item) 
{
...
void 
tree_monit::visit_simple_for_command (tree_simple_for_command& item) 
{
...
void 
tree_monit::visit_complex_for_command (tree_complex_for_command& item) 
{
...
void 
tree_monit::visit_while_command (tree_while_command& item) 
{
...
void 
tree_monit::visit_do_until_command (tree_do_until_command& item) 
{
...
void 
tree_monit::visit_if_command (tree_if_command& item) 
{
...
void 
tree_monit::visit_if_command_list (tree_if_command_list& item)
{
  int i = 0;

  tree_if_command_list::iterator p = item.begin();
...
void 
tree_monit::visit_if_clause (tree_if_clause& item)
{
...
void 
tree_monit::visit_switch_command (tree_switch_command& item)
{
...
void 
tree_monit::visit_switch_case_list (tree_switch_case_list& item)
{
  int i = 0;
  tree_switch_case_list::iterator p = item.begin();
  while(p!=item.end()){
...
void 
tree_monit::visit_switch_case (tree_switch_case& item)
{
...
void 
tree_monit::visit_decl_command (tree_decl_command& item)
{
...
void 
tree_monit::visit_decl_init_list (tree_decl_init_list& item)
{
  int i = 0;
  tree_decl_init_list::iterator p = item.begin();
  while(p!=item.end()){
    tree_decl_elt *elt = *p++;
...
void 
tree_monit::visit_decl_elt (tree_decl_elt& item)
{
...
void 
tree_monit::visit_plot_command (tree_plot_command& item)
{
...
void 
tree_monit::visit_try_catch_command (tree_try_catch_command& item)
{
...
void 
tree_monit::visit_unwind_protect_command (tree_unwind_protect_command& item)
{
...
void 
tree_monit::visit_binary_expression (tree_binary_expression& item)
{
...
void 
tree_monit::visit_colon_expression (tree_colon_expression& item)
{
...
void 
tree_monit::visit_constant (tree_constant& item)
{
...
void 
tree_monit::visit_identifier (tree_identifier& item)
{
  // fprintf (f, "%*s*(tree_identifier *)%8.8p name=\"%s\" record=%8.8p\n", 
  // 	   indent, "", &item,
  // 	   item.name ().c_str (),
  // 	   item.symbol().sym);
  fprintf (f, "%*s*(tree_identifier *)%8.8p name=\"%s\" \n", 
	   indent, "", &item,
	   item.name ().c_str ());
}

void 
tree_monit::visit_index_expression (tree_index_expression& item)
{
  ...
  fprintf (f, "%*sargument list (todo):\n", indent, "");
  indent += 2;
  std::list<tree_argument_list *> arg_lists = item.arg_lists ();
  std::list<tree_argument_list *>::iterator list =  arg_lists.begin();
  if (*list)
    (*list)->accept (*this);
  indent -= 2;
  indent -= 2;
...
void 
tree_monit::visit_argument_list (tree_argument_list& item) 
{
  int i = 0;
  tree_argument_list::iterator p = item.begin();
  while(p!=item.end()) {
...
void 
tree_monit::visit_simple_assignment (tree_simple_assignment& item) 
{
...
void 
tree_monit::visit_multi_assignment (tree_multi_assignment& item) 
{
  fprintf (f, "%*s*(tree_multi_assignment *)%8.8p \n", indent, "", 
	   &item);
  
  indent += 2;
  octave_value::assign_op op = item.op_type();
  fprintf (f, "%*soperator: %d(%s)\n", indent, "",
	   op, octave_value::assign_op_as_string (op).c_str());
...
// void 
// tree_monit::visit_indirect_ref (tree_indirect_ref& item) 
// {
...
void 
tree_monit::visit_matrix (tree_matrix& item) 
{
  fprintf (f, "%*s*(tree_matrix *)%8.8p\n", indent, "", 
	   &item);

  int i = 0;
  tree_matrix::iterator p = item.begin();
  while(p!=item.end()){
...
void 
tree_monit::visit_cell (tree_cell& item) 
{
  fprintf (f, "%*s*(tree_cell *)%8.8p\n", indent, "", 
	   &item);

  int i = 0;
  tree_cell::iterator p = item.begin();
  while(p!=item.end()){
    tree_argument_list *elt = *p++;
    if (elt) {
      fprintf (f, "%*s%d. row:\n", 
...
void  
tree_monit::visit_postfix_expression (tree_postfix_expression& item) 
{
...
void  
tree_monit::visit_prefix_expression (tree_prefix_expression& item) 
{
  fprintf (f, "%*s*(tree_postfix_expression *)%8.8p \n", indent, "", 
	   &item);
