...
#define ARRAY_DEPTH_NESTING_INCREMENT 1
#define RECORD_DEPTH_NESTING_INCREMENT 1
...
#if 0
    for (Context::Binding * binding = _context->_bindings;
...
# endif /* RNA_TYPECHECK */
	...
	if (rna._word.record) {
	    ...
	    if (entry->depth_nesting == -1
		|| side_nesting < entry->side_nesting
		|| (side_nesting == entry->side_nesting
		    && depth_nesting < entry->depth_nesting)) {
		entry->side_nesting = side_nesting;
		entry->depth_nesting = depth_nesting;
		resolved = false;
	    }
...
# endif /* RNA_TYPECHECK */
		    scan_item (item, rna._word.record->_typeinfo->type(i),
			       side_nesting, depth_nesting+RECORD_DEPTH_NESTING_INCREMENT);
		}
		for (List * s = rna._word.record->_attr; s; s = s->_cdr) {
		    scan_item (s->_car, s->_type, side_nesting+1, 0);
		}
		break;
	    case ARRAY:
...
# endif /* RNA_TYPECHECK */
		    scan_item (item, rna._word.array->_type, 
			       side_nesting, depth_nesting+ARRAY_DEPTH_NESTING_INCREMENT);
		}
		break;
	    case STR:
		/* do nothing */;
...
# endif /* RNA_TYPECHECK */
	...
	if (rna._word.ss) {
		...
		&& side_nesting == entry->side_nesting
		&& depth_nesting == entry->depth_nesting) {
		_context->resolve (entry, rna._word);
		entry->type = type;
...
# else /* RNA_TYPECHECK */
	    Rna item (rna._word.record->_item [i], rna._word.record->_typeinfo->type(i));
# endif /* RNA_TYPECHECK */
	    write_item (item, rna._word.record->_typeinfo->type(i), sns, 
			side_nesting, depth_nesting+RECORD_DEPTH_NESTING_INCREMENT);
	}
	write_attr_list (rna._word.record->_attr, sns, side_nesting+1, 0);
	sns.unindent (")");
	break;
    case ARRAY:
	sns.indent ("[");
...
# else /* RNA_TYPECHECK */
	    Rna item (rna._word.array->_item [i], rna._word.array->_type);
# endif /* RNA_TYPECHECK */
	    write_item (item, rna._word.array->_type, sns, 
			side_nesting, depth_nesting+ARRAY_DEPTH_NESTING_INCREMENT);
	    if (i < length-1)
		sns.separator ();
	}
				  ...
				  Int side_nesting, Int depth_nesting)
{
	...
	sns.separator ();
	sns.printf ("%s=", s->_tag);
	write_item (s->_car, s->_type, sns, side_nesting, depth_nesting);
    }
